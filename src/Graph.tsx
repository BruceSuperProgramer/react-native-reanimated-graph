...
import { GRAPH_PROPS, SIZE } from "./config";
...
const Graph = ({
  ...
  useDerivedValue(() => {
    ...
    if (onCursorMove) onCursorMove({ x: translation.x.value, y });
  });
  ...
  return (
    <View style={styles.container}>
      <Svg width={canvasWidth} height={canvasHeight}>
        ...
        <AnimatedPath
          animatedProps={animatedGradientBackgroundProps}
          fill="url(#gradient)"
        />
      </Svg>
      ...
      <Tabs
        transition={transition}
        tabNames={graphs.map((o) => o.graphName)}
        customContainerStyle={tabsCustomContainerStyle}
        canvasWidth={canvasWidth}
        previousSelectionValue={previous}
        currentSelectionValue={current}
      />
    </View>
  );
};
